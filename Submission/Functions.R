...
sampleClusterResponses <- function(responseClusters){
  ...
  #Rename columns of dataframe for formatting
  sampleResponses <- sampleResponses %>%
    dplyr::rename(Responses = "responses", Clusters = "clusters")
  
  #Remove rownames for formatting
  row.names(sampleResponses) <- NULL
...
# Required Packages:  magrittr, dplyr
# Input:              Responses, the calculated sentiment
# Output:             A data frame of the top 5 most positive sentiments and top 5 most negative sentiments
getExtremeSentiments <- function(vectorOfResponses, sentiment){
  #Turn arguments into a data frame
  dfResponses <- as.data.frame(vectorOfResponses)
  dfSentiment <- as.data.frame(sentiment$ave_sentiment)
  responsesWithSentiment <- cbind(dfResponses, dfSentiment) %>%
    dplyr::arrange(desc(sentiment$ave_sentiment))
  
  #Get the number of responses
  numResponses <- nrow(responsesWithSentiment)
  
  #Get the responses with the max and min sentiement
  maxAndMinSentimentResponses <- responsesWithSentiment[c(1:5, (numResponses-4):numResponses),]

  #Remove row names for formatting
  row.names(maxAndMinSentimentResponses) <- NULL
  
  return(maxAndMinSentimentResponses)
}
